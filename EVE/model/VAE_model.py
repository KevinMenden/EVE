...
class VAE_model(nn.Module):
    ...
    def __init__(
        ...
        self.encoder = VAE_encoder.VAE_MLP_encoder(
            params=encoder_parameters, device=device
        )
        if decoder_parameters["bayesian_decoder"]:
            self.decoder = VAE_decoder.VAE_Bayesian_MLP_decoder(
                params=decoder_parameters, device=device
            )
        else:
    ...
    def compute_evol_indices(
        ...
        list_valid_mutated_sequences[
            "wt"
        ] = msa_data.focus_seq_trimmed  # first sequence in the list is the wild_type
        ...
        with torch.no_grad():
            for i, batch in enumerate(
                ...
                for j in tqdm.tqdm(
                    ...
                    prediction_matrix[
                        i * batch_size : i * batch_size + len(x), j
                    ] = seq_predictions
                tqdm.tqdm.write("\n")
